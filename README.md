...
4. 手机进入同一段对话，可从相册选择静态图片、可选本地语音转文字、编辑并显式发送。
...
- 同一会话的文本聊天、Markdown、流式回复与服务端受控静态图片展示
- 服务声明 `baton/1.3` 的 `image_upload` 时，可从相册选择静态图片，查看缩略图/预览，并显式发送图文或纯图消息；图片以同源上传后通过 `image_ref` 原子提交，取消与同一草稿的显式幂等重试均受支持
- 停止生成、断线恢复与共享会话结束
- iOS 相机扫码；Simulator 的 Debug 版本支持本地演示服务
- iOS 本地 Speech-to-Text：转写后可编辑，再作为普通文本发送

当前刻意不做相机、文件、视频或任意外链图片输入，以及 Tool UI、Agent action approval、推送、位置与生成式 UI。相册静态图片输入已按 V1.3 契约实现：服务明确声明能力后，用户才能从相册选择、显式发送；服务验证并保存后，它会作为同一 Conversation 的正常消息同步给 Web 和 iPhone。自动化收口已完成；真机相册、后台与文件保护验收仍在进行中。Baton 的核心是先把“跨设备接力同一段对话”做到可靠。
